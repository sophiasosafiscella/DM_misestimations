...
def whiten_resids(fitter, restype='postfit'):
    """
    Function to whiten original_residuals. If no reddened original_residuals, input will be returned.
    ...
    Returns:
    ---------
    wres [array]: Array of whitened timing original_residuals.
    """
    ...
    if type(fitter) is dict:
        ...
        # Now check if red noise original_residuals
        if "pl_red_noise" in noise_rs:
            wres = rs - noise_rs['pl_red_noise']
        else:
            log.warning("No red noise, original_residuals already white. Returning input original_residuals...")
            wres = rs
    # if not assume it's a PINT fitter class object
        ...
        if fitter.is_wideband:
            ...
            # Get number of original_residuals
...
def rms_by_backend(resids, errors, rcvr_backends, dm = False):
    """
    Function to take a fitter, list of original_residuals errors, and backends and compute the rms and weighted rms
    for either time original_residuals or DM original_residuals if a wideband original_residuals.

    Inputs:
    ----------
    resids [list]: List of original_residuals.
    errors [list]: List of residual errors.
    rcvr_backends [list]: List of backends.
    dm [boolean]: If True, will do computation with DM original_residuals [defaut: False].
    
    Returns:
    ----------
    rs_dict [dictionary]: Dictionary of rms and wieghted rms original_residuals for each backend-reciever combination.
    """
    # Define output dictionary
    rs_dict = {}
    # Get RMS of all original_residuals
    ...
    if dm:
        ...
        # Get the weighted rms of averaged original_residuals
...
def resid_stats(fitter, epoch_avg = False, whitened = False, dm_stats = False, print_pretty = False):
    """
    Function to get statistics for the original_residuals. This includes the RMS and WRMS for all original_residuals, as well as
    ...
    epoch_avg [boolean]: If True, will output stats for epoch averaged residulas, else will be for 
        non-epoch averaged original_residuals [default: False].
    whitened [boolean]: If True, will output stats for whitened residulas, else will be for 
        non-whitened original_residuals [default: False].
    dm_stats [boolean]: If True, will also output the stats for the DM original_residuals for wideband fitters.
    ...
    rs_dict [nested dictionary]:
        ...
        First set of keys are Receiver-Backend combos, e.g. L-wide_ASP, S-wide_PUPPI. For the W/RMS of all original_residuals,
        the key is 'All'.

        Within each Receiver-Backend combo the flags are:
            rms [astropy quantity: rms of the original_residuals for the receiver-backend combo in microseconds.
            wrms [astropy quantity: weighted rms of the original_residuals for the receiver-backend combo in microseconds.
     
    dm_dict [nested dictionary]: Same as rs_dict but for DM original_residuals with unit of pc cm^-3.
    """
    ...
    if fitter.is_wideband:
        ...
        NB = False
        if epoch_avg:
            log.warning("Warning, cannot epoch average wideband original_residuals, will skip epoch averaging.")
    else:
        resids = fitter.resids
        NB = True

    # get rcvr backend combos for averaged original_residuals
    ...
    if dm_stats:
        if not NB:
            ...
            log.warning("Cannot compute DM Stats, not Wideband timing data_res_avg.")
...
def report_ptest(label, ftest_dict = None, alpha=ALPHA):
    ...
    # Else print the computed values
        ...
        rms = ftest_dict['resid_wrms_test'].value  # weighted root mean square of timing original_residuals
        ...
        if "dm_resid_wrms_test" in ftest_dict.keys():
            dmrms = ftest_dict['dm_resid_wrms_test'].value  # weighted root mean square of DM original_residuals
        else:
            dmrms = None
...
def pdf_writer(fitter, 
    ...
    parfile [string]: Name of parfile used to generate original_residuals.
    ...
    for m in mjdlist:
        if m > m0 + maxepoch:
            nepoch += 1
            m0 = m
    fsum.write('Epochs (defined as observations_in_window within %.1f-day spans): %d\\\\\n' % (maxepoch, nepoch))

    # Print what fitter was used:
    fsum.write('Wideband data_res_avg: %s\n' % ('No' if NB else 'Yes'))
    ...
    fsum.write(r'\subsection*{Epochs near center of data_res_avg span?}' + '\n')
    tmidspan = 0.5*(float(finish)+float(start))
    fsum.write('Middle of data_res_avg span: midspan = %.2f\\\\\n' % (tmidspan))
    dtpepoch = float(model.PEPOCH.value)-tmidspan
    fsum.write('PEPOCH - midspan = $%.2f$ days = $%.2f$ years\\\\\n'  % ( dtpepoch, dtpepoch/365.24))
    if abs(dtpepoch) > 1:
        fsum.write(alert('PEPOCH is not at center of data_res_avg span') + "\\\\\n")
    if param_check('TASC', fitter, check_enabled=True):
        ...
        if abs(dttasc) > 1 + porb:
            fsum.write(alert('TASC is not at center of data_res_avg span') + "\\\\\n")
    if param_check('T0', fitter, check_enabled=True):
        dtt0 = float(model.T0.value)-tmidspan
        fsum.write('TASC - midspan = $%.2f$ days = $%.1f$ years\\\\\n'  % ( dtt0, dtt0/365.24))
        if abs(dtt0) > 1 + porb:
            fsum.write(alert('T0 is not at center of data_res_avg span') + "\\\\\n")
    fsum.write('\n')
    ...
    if False:
        ...
        if not is_wideband:
            ...
            fsum.write('No fractional bandwidth check for DMX ranges with wideband data_res_avg!\\\\\n')
        """
        if not NB:
            fsum.write('No fractional bandwidth check for DMX ranges with wideband data_res_avg!\\\\\n')
