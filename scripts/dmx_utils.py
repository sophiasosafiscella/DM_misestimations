...
def get_gasp_dmx_ranges(toas, group_width=0.1, bin_width=15.0, pad=0.0,
    ...
    # Check that the TOAs contain GASP data_res_avg
    bes = np.array(toas.get_flag_value('be')[0])  # backend flag
    if 'GASP' not in set(bes):
        return []

    # Only consider GASP data_res_avg outside the GUPPI range (PUPPI came later)
    ...
    # Pair up high and low frequency GASP observations_in_window
...
def remove_some_dmx_ranges(model, toas_MJD, quiet=False):
    ...
    dmx_ranges, dmx_vals, dmx_errs = model_dmx_params(model) # Get the range of each DMX_xxx parameter
