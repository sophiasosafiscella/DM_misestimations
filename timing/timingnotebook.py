...
class TimingNotebook:
    ...
    def add_prenoise(self,filename="config.yaml",
        ...
        self.add_markdown_cell_skip('''\
        Run basic checks for pulsar name, solar system ephemeris, clock correction, ecliptic coordinates, tropospheric delays, planet Shapiro delays, and if applicable, removal of Arecibo data_res_avg affected by bad LO. Check that TOAs being used are from the latest `toagen` release. Also check for the appropriate number of receiver JUMPs and DMJUMPs and fix them automatically if necessary.\
        ''', autorun)
        self.add_code_cell('''\
        ...
        if not autorun:
            ...
            Ensure DMX windows are calculated properly for the current set of TOAs, set non-binary epochs to the center of the data_res_avg span and print a summary of TOAs included.\
            ''')
            self.add_code_cell('''\
            ...
            ''')
            self.add_markdown_cell('''\
            Define the fitter object and inspect prefit original_residuals vs. MJD (also vs. orbital phase for binary MSPs).\
            ''')
            self.add_code_cell('''\
            ...
            ''')
            self.add_markdown_cell('''\
            Check that free parameters follow convention, do the fit, plot post-fit original_residuals, write a pre-noise par file and print a summary of fit results.\
            ''')
            self.add_code_cell('''\
    ...
    def add_noise(self,run_noise=False,use_existing=False,autorun=False,write=False):
        ...
        Noise analysis runs are required for the 15-yr v0.9 data_res_avg set, using the latest available timing model and set of TOAs. Once a new `.yaml` file is merged, noise runs will get kicked off on Thorny Flats (HPCs) using the `timing-model` and `toas` indicated therein. For pulsars without existing pre-noise solutions, timers should use the [prenoise] section of this notebook to improve TOA excision, ensure original_residuals are flat, then submit a merge request with that solution, and wait for noise results to be made available before proceeding.
    ...
    def add_significance(self,autorun=False):
        ...
        Get information on the weighted (W)RMS original_residuals per backend. Set `epoch_avg = True` to get the (W)RMS of the epoch-averaged original_residuals (does not work for wideband analysis; the timing model must have `ECORR` in order for epoch averaging to work). Set `whitened = True` to get the (W)RMS of the whitened original_residuals. Set both to `True` to get the (W)RMS of the whitened, epoch-averaged original_residuals.

        For wideband analysis, set `dm_stats = True` to also return the (W)RMS of the DM original_residuals.\
        ''', autorun)
        self.add_code_cell('''\
