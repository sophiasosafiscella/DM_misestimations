...
def large_residuals(fo,threshold_us,threshold_dm=None,*,n_sigma=None,max_sigma=None,prefit=False,ignore_ASP_dms=True,print_bad=True):
    """Quick and dirty routine to find outlier original_residuals based on some threshold.
    ...
    threshold_us: float
        not a quantity, but threshold for TOA original_residuals larger (magnitude) than some delay in microseconds; if None, will not look at TOA original_residuals
    threshold_dm: float
        not a quantity, but threshold for DM original_residuals larger (magnitude) than some delay in pc cm**-3; if None, will not look at DM original_residuals
    ...
    ignore_ASP_dms: bool
        If True, it will not flag/excise any TOAs from ASP or GASP data_res_avg based on DM criteria
    print_bad: bool
    ...
    # check if using wideband TOAs, as this changes how to access the original_residuals
...
def display_auto_ex(tc, mo, cutkeys=['epochdrop', 'outlier10'], plot_type='profile'):
    ...
    ff_list = sorted(glob.glob('/nanograv/timing/releases/15y/toagen/data_res_avg/*/*/*.ff'))
...
def highlight_cut_resids(toas,model,tc_object,cuts=['badtoa','badfile'],multi=False,ylim_good=True,save=True):
    """ Plot original_residuals vs. time, highlight specified cuts (default: badtoa/badfile)
    ...
    cuts: list, optional
        cuts to highlight in original_residuals plot (default: manual cuts)
    multi: bool, optional
...
def check_tobs(toas,required_tobs_yrs=2.0):
    """ Throws a warning if epoch timespan is insufficient
...
def file_look(filenm, plot_type = 'profile'):
    ...
    Parameters
    ==========
    filenm: file name of epoch of interest
...
def dmx_mjds_to_files(mjds,toas,dmxDict,mode='nb',file_only=False):
    ...
    Tool to facilitate the process of inspecting data_res_avg associated with outlier
    ...
    """
    match_files = []
    ff_path = sorted(glob.glob('/nanograv/timing/releases/15y/toagen/data_res_avg/*/*/*.ff'))
